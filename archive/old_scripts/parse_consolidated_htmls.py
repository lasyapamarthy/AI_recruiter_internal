#!/usr/bin/env python3
"""
Parser for consolidated LinkedIn HTML files.
This parser works with the consolidated_htmls directory that contains
only files with experience data from all three source directories.
"""
import glob
import json
import re
...
def extract_url_from_filename(filename):
    ...
    # Remove source directory prefix first
    if filename.startswith('collected_htmls_'):
        filename = filename.replace('collected_htmls_', '')
    elif filename.startswith('htmls_'):
        filename = filename.replace('htmls_', '')
    elif filename.startswith('safari_htmls_'):
        filename = filename.replace('safari_htmls_', '')
...
def get_source_directory_from_filename(filename):
    """Extract source directory from consolidated filename."""
    if filename.startswith('collected_htmls_'):
        return 'collected_htmls'
    elif filename.startswith('htmls_'):
        return 'htmls'
    elif filename.startswith('safari_htmls_'):
        return 'safari_htmls'
...
def parse_consolidated_profile(path: pathlib.Path) -> dict:
    """Parse consolidated LinkedIn profile HTML files."""
    try:
...
def main():
    """Main function to process all consolidated HTML files."""
    
    # Define the consolidated directory
    consolidated_dir = "data/linkedin/output/consolidated_htmls"
    
    if not os.path.exists(consolidated_dir):
        print(f"Consolidated directory does not exist: {consolidated_dir}")
        print("Please run consolidate_and_filter_htmls.py first")
        return
    
    # Find all HTML files in consolidated directory
    html_files = list(pathlib.Path(consolidated_dir).glob("*.html"))
    
    print(f"Found {len(html_files)} HTML files in consolidated directory")
    ...
    for i, html_file in enumerate(html_files, 1):
        ...
        result = parse_consolidated_profile(html_file)
        result['source_directory'] = get_source_directory_from_filename(html_file.name)
    ...
    output_file = 'consolidated_profiles_parsed.csv'
    ...
    # Print summary statistics by source directory
    print(f"\nSummary by source directory:")
    source_dirs = ['collected_htmls', 'htmls', 'safari_htmls']
    for source_dir in source_dirs:
        dir_results = [r for r in results if r['source_directory'] == source_dir]
    ...
    successful_examples = [r for r in results if not r['experience_empty']][:10]
    ...
    profiles_with_exp = [r for r in results if not r['experience_empty']]
    
    if profiles_with_exp:
        exp_output_file = 'consolidated_profiles_with_experience.csv'
