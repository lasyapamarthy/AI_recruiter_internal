#!/usr/bin/env python3
"""
Modified version of LinkedIn parser that:
1. Creates a CSV file with all results
2. Creates a separate TXT file listing profiles where start date is missing for the most recent job

The script produces two files:
1. A CSV with all profile data (same as original parser)
2. A TXT file containing LinkedIn URLs of profiles with missing start dates

Usage
-----
python parser_with_missing_dates.py /path/to/html_folder \
       --out my_experience_dump.csv \
       --missing-dates missing_dates.txt
"""
...
# Main driver                                                                 #
# --------------------------------------------------------------------------- #
def run(indir: Path, outfile: Path, missing_dates_file: Path) -> None:
    ...
    text_count = 0
    rows = []
    missing_dates = []  # List to store profiles with missing dates
    ...
    for html_path in html_files:
        ...
        if rec:
            ...
            # Track profiles with missing start dates
            if not rec["start_date"].strip():
    ...
    print(f"Found {len(missing_dates)} profiles with missing start dates")
    ...
    # Save missing dates file
    with missing_dates_file.open("w", encoding="utf-8") as f:
        f.write("Profiles with missing start dates for current position:\n\n")
        for profile in missing_dates:
            f.write(f"LinkedIn URL: {profile['linkedin_url']}\n")
            if profile['name']:
                f.write(f"Name: {profile['name']}\n")
            if profile['job_title']:
                f.write(f"Current Title: {profile['job_title']}\n")
            if profile['company']:
                f.write(f"Current Company: {profile['company']}\n")
            f.write("\n")  # Blank line between profiles
            
    print(f"✓ Saved {len(missing_dates)} profiles with missing dates to {missing_dates_file}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=(
            ...
            "LinkedIn profile HTML files and track profiles with missing dates."
        )
    )
    parser.add_argument(
        "indir",
    ...
    parser.add_argument(
        ...
        "--missing-dates",
        type=Path,
        default=Path("missing_start_dates.txt"),
        help="Output file for profiles with missing dates (default: missing_start_dates.txt)",
    )
    args = parser.parse_args()
    run(args.indir, args.out, args.missing_dates) 